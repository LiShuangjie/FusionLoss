...
class PredictModel(nn.Module):
    ...
    #     def _d(x, src, tgt):
    #         if self.conf.model['dist_method'] == 'cos':
    #             # d = (x[src] * x[tgt]).sum(dim=-1)
    #             d = F.cosine_similarity(x[src], x[tgt])
    #         elif self.conf.model['dist_method'] == 'norm2':
    ...
    #     nnodes =  x.shape[0]
    #     src, tgt = edge_index
    #     localsim = scatter_mean(dist, tgt, out=torch.zeros([nnodes]).to(edge_index.device))
    #     localsim_square = scatter_mean(dist.square(), tgt, out=torch.zeros([nnodes]).to(edge_index.device))

    #     return localsim, localsim_square
    ...
    # def count_same_label_neighbors_fast(self, A, y):
    #     # 创建标签相等的掩码矩阵
    #     y = y.cpu()
    #     label_eq = (y[:, None] == y[None, :]).int()
    #     # 相乘后统计标签相同且有边的数量
    #     same_count = int((A * label_eq).sum())
    #     total_count = int(A.sum())
    #     print("percent",same_count/total_count)
    #     breakpoint()
    # def count_same_label_neighbors(self, A, y):
    #     # A 是 n x n 的邻接矩阵，y 是长度为 n 的标签向量
    #     n = A.shape[0]
    #     y = y.cpu()
    #     count = 0
    #     for i in range(n):
    ...
    #                 count += 1
    #     total_count = int(A.sum())
    #     print("count",count, count/total_count)
...
class LabelDivision(nn.Module):
    ...
    def division(self, idx_train, emb_lp, emb_hp,label_noise,localsim, epoch,binary_y, noise_rate=None):
        ...
        # rate_schedule = np.ones(self.conf.training['n_epochs'])*forget_rate
        # rate_schedule[:70] = np.linspace(0, forget_rate**1, 70)
        ...
        # # 消融 w/o cf 全部的ind_update_1都看作noisy
        # loss_noise_lp =  F.cross_entropy(z_lp[ind_update_1], y_train_noise[ind_update_1], reduction='none').float()
        # loss_noise_hp = F.cross_entropy(z_hp[ind_update_1], y_train_noise[ind_update_1], reduction='none').float()
        ...
        # 0.4
        # loss_label = 0.001*loss_noise + loss_clean + loss_dc
        # 消融 w/o re
        # loss_label = loss_clean + loss_dc
        # 消融 w/o cf
        # loss_label =  0.1*loss_noise + loss_clean
        # 消融 w/ cl
