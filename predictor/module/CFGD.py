...
class Dual_GCN(nn.Module):
    def __init__(self, nfeat, nhid, nclass, dropout=0.5, lr=0.001, weight_decay=5e-4, with_relu=True, with_bias=True,
        ...
        # if self.arxiv:
        #     self.gc2_1 = GCNConv(nhid, nhid, bias=with_bias, add_self_loops=self_loop)
        # else:
        self.gc2_1 = GCNConv(nhid, nclass, bias=with_bias, add_self_loops=self_loop) 
        # self.gc3_1 = GCNConv(nhid, nclass, bias=with_bias, add_self_loops=self_loop)

        self.gc1_2 = GCNConv(nfeat, nhid, bias=with_bias, add_self_loops=self_loop)
        # if self.arxiv:
        #     self.gc2_2 = GCNConv(nhid, nhid, bias=with_bias, add_self_loops=self_loop)
        # else:
        self.gc2_2 = GCNConv(nhid, nclass, bias=with_bias, add_self_loops=self_loop)
        # self.gc3_2 = GCNConv(nhid, nclass, bias=with_bias, add_self_loops=self_loop)
    ...
    def forward(self, x, edge_index, edge_weight=None):
        if self.with_relu:
            x1_emb = F.relu(self.gc1_1(x, edge_index, edge_weight))
        else:
            x1_emb = self.gc1_1(x, edge_index, edge_weight)

        x1 = F.dropout(x1_emb, self.dropout, training=self.training)
        x1 = self.gc2_1(x1, edge_index, edge_weight)
        # if self.arxiv:
        #     # arxiv 采用3层GCN
        #     x1 = self.gc3_1(x1, edge_index, edge_weight)
        ...
        # if self.arxiv:
        #     # arxiv 采用3层GCN
        #     x2 = self.gc3_1(x2, edge_index, edge_weight)
